...
window.Admin={
  ...
  render(){
    ...
    ${
      ...
      <main class="admin-main">
        ...
        <section id="section-products" class="admin-section"><div class="section-toolbar"><div><h2>Produk & Kategori</h2><p>Kelola produk dan kategori yang tersinkron ke JSONBin.</p></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="primary-button" onclick="Admin.addCategory()"><i class="fa-solid fa-tags"></i>Tambah Kategori</button><button class="primary-button" onclick="Admin.addProduct()"><i class="fa-solid fa-plus"></i>Tambah Produk</button></div></div><div id="categoryList" style="display:flex;gap:10px;flex-wrap:wrap;margin:0 0 20px"></div><div id="productList" class="product-admin-list"></div></section>
        ...
        <section id="section-information" class="admin-section"><div class="section-toolbar"><div><h2>Informasi Website</h2><p>Kelola informasi dan status toko yang tampil di halaman pelanggan.</p></div></div>
  ...
  open(section,button){
    ...
    $("#pageTitle").textContent={dashboard:"Dashboard",products:"Produk",orders:"Transaksi",information:"Informasi",settings:"Pengaturan"}[section]||"Admin";
    this.closeSidebar();
  },
  renderAll(){this.renderStats();this.renderCategories();this.renderProducts();this.renderOrders();this.renderSettings();this.renderInformation()},
  ...
  openProductModal(product=null){
        ...
        <div style="width:min(720px,100%);max-height:92vh;overflow:auto;background:#0b1628;border:1px solid rgba(80,170,255,.25);border-radius:20px;box-shadow:0 25px 80px rgba(0,0,0,.5);color:#fff;font-family:Inter,Arial,sans-serif">
          ...
          <form id="yamzzProductForm" style="padding:24px">
            <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px">
              ...
              <div style="grid-column:1/-1"><label style="display:block;font-size:13px;font-weight:700;margin-bottom:8px">Deskripsi</label><textarea id="pmDescription" rows="4" placeholder="Deskripsi produk..." style="width:100%;box-sizing:border-box;padding:13px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.1);background:#07111f;color:#fff;outline:none;resize:vertical">${esc(product?.description||'')}</textarea></div>\n              <div style="grid-column:1/-1"><label style="display:block;font-size:13px;font-weight:700;margin-bottom:8px">Link Produk <span style="font-weight:500;color:#7f95ad">(Opsional)</span></label><input id="pmDeliveryLink" type="url" value="${esc(product?.deliveryLink||'')}" placeholder="https://contoh.com/link-produk" style="width:100%;box-sizing:border-box;padding:13px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.1);background:#07111f;color:#fff;outline:none"><div style="font-size:11px;color:#7f95ad;margin-top:7px;line-height:1.6">Jika diisi, setelah pembayaran <b style="color:#b8c9dc">PAID</b> pelanggan akan mendapat tombol link produk. Jika kosong, popup hanya memberi info bahwa pesanan diproses admin.</div></div>
            </div>
    ...
    modal.querySelector('#yamzzProductForm').onsubmit=async(e)=>{
      ...
      if(deliveryLink){try{new URL(deliveryLink);}catch{error.textContent='Link Produk tidak valid. Gunakan URL lengkap, contoh https://...';error.style.display='block';return;}}
      ...
      try{
        ...
        const data={name,category,ress,price,stock,image,description,deliveryLink};
  ...
  async saveInformation(){
    ...
    DB.site={...DB.site,notification:val("setNotification"),banner:val("setInfoBanner"),maintenance:$("#setMaintenance")?.checked===true,storeOffline:$("#setStoreOffline")?.checked===true};
    await this.commit("Informasi website disimpan");
  },
  renderSettings(){
    const s=DB.site;
  ...
  async saveSettings(){
    const val=id=>$("#"+id)?.value.trim()||"";
    DB.site={...DB.site,name:val("setName")||"Yamzz Market",title:val("setTitle")||`${val("setName")||"Yamzz Market"} • JASTEB`,tagline:val("setTagline"),description:val("setDescription"),whatsapp:val("setWhatsapp"),email:val("setEmail"),whatsappChannel:val("setWhatsappChannel"),logo:val("setLogo"),banner:val("setBanner"),cloudinaryCloudName:val("setCloudName"),cloudinaryUploadPreset:val("setUploadPreset"),socials:{tiktok:val("setTiktok"),instagram:val("setInstagram"),youtube:val("setYoutube"),telegram:val("setTelegram")},notification:DB.site.notification||"",maintenance:DB.site.maintenance===true,storeOffline:DB.site.storeOffline===true};

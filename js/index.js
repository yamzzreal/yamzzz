...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Monitor transaksi PAID secara berkala dari JSONBin.
...
/* =========================================================
   LIVE TRANSACTIONS
   Menampilkan transaksi yang memang sudah tersimpan di JSONBin.
...
async function refreshPaidTransactions(){
    try{
        ...
        const signature=transactions.map(order=>String(
            order.id||order.casakuTransactionId||`${order.name}-${order.product}-${order.createdAt}`
        )).join("|");
